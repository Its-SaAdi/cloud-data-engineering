...
WITH cte_store_sales (store_id, product_id, total_quantity, sales_rank)
AS
(
    SELECT
        o.store_id,
...
WITH cte_price_rank (category_id, product_name, list_price, price_rank)
AS
(
    SELECT
        category_id,
...
WITH cte_duplicates (customer_id, first_name, last_name, phone, city, row_num)
AS
(
    SELECT
        customer_id,
...
WITH cte_monthly_sales (month, net_sales)
AS
(
    SELECT
...
SELECT
    ...
    ROUND(LAG(net_sales, 1) OVER (ORDER BY month), 2) AS previous_month_sales,
    ROUND(net_sales - LAG(net_sales, 1) OVER (ORDER BY month), 2) AS month_over_month_diff
