...
WITH cte_category_avg (category_id, avg_price)
AS
(
    SELECT
        category_id,
...
INNER JOIN cte_category_avg ca
...
SELECT
    so.staff_id,
    so.order_count
FROM cte_staff_orders so
CROSS JOIN cte_avg_orders ao
...
WITH cte_store_revenue (store_id, order_year, total_revenue)
AS
(
    SELECT
        o.store_id,
...
ADD CONSTRAINT chk_shipped_date_after_order
